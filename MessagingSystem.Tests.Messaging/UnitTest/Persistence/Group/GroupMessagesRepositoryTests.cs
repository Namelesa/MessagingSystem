...
public class GroupMessagesRepositoryTests : IDisposable
{
    ...
    [Fact]
    public async Task GetMessageStoryAsync_WhenMessagesExist_ReturnsOrderedMessages()
    {
        // Arrange
        ...
        var groupGuid = Guid.Parse("00000000-0000-0000-0000-000000000001");
        var groupGuid1 = Guid.Parse("00000000-0000-0000-0000-000000000002");
        
        var message1 = new GroupMessage("Sender1", "Content 1")
        {
            Id = Guid.NewGuid(),
            GroupId = groupGuid,
            SendTime = baseTime.AddMinutes(-10)
        };
        message1.SetHashes("testHash123");
        
        var message2 = new GroupMessage("Sender2", "Content 2")
        {
            Id = Guid.NewGuid(),
            GroupId = groupGuid,
            SendTime = baseTime.AddMinutes(-5)
        };
        message2.SetHashes("testHash456");
        ...
        message3.SetHashes("testHash789");
        
        var messageOtherGroup = new GroupMessage("SenderOther", "Content Other")
        {
            Id = Guid.NewGuid(),
            GroupId = groupGuid1,
            SendTime = baseTime
        };
        messageOtherGroup.SetHashes("testHashOther");

        await _context.GroupMessages.AddRangeAsync(message1, message2, message3, messageOtherGroup);
        await _context.SaveChangesAsync();
        ...
        Assert.Equal(message3.Id, result[0].Id);
        Assert.Equal(message2.Id, result[1].Id);
        Assert.Equal(message1.Id, result[2].Id);
    }
