...
public class DeleteUserInfoConsumerTests
{
    ...
    public async Task Consume_WhenPublicKeyNotFound_ShouldLogWarningAndReturn()
    {
        ...
        var request = new DeleteUserInfoRequest(TestUserNickNameHash);
    ...
    public async Task Consume_WhenAllOperationsSucceed_ShouldReturnSuccessResponse()
    {
        ...
        _contextMock.Verify(x => x.RespondAsync(It.Is<DeleteUserInfoRollback>(r => 
            r.UserNickNameHash == TestNickName && r.IsSuccess == true)), Times.Once);

        VerifyAllOrchestratorsCalled();
        VerifyEncryptionCalled();
    }
    ...
    public async Task Consume_WhenSomeOperationsFail_ShouldReturnFailureResponseAndLogErrors()
    {
        ...
        _loggerMock.Verify(
            x => x.Log(
                LogLevel.Error,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, t) => v.ToString().Contains("Member operation failed")),
                It.IsAny<Exception>(),
                It.IsAny<Func<It.IsAnyType, Exception, string>>()),
            Times.Once);
        ...
        _loggerMock.Verify(
            x => x.Log(
                LogLevel.Error,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, t) => v.ToString().Contains("User operation failed")),
                It.IsAny<Exception>(),
                It.IsAny<Func<It.IsAnyType, Exception, string>>()),
            Times.Once);
    }
    ...
    public async Task Consume_WhenMemberTaskHasData_ShouldLogInformation()
    {
        ...
        _loggerMock.Verify(
            x => x.Log(
                ...
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, t) => v.ToString().Contains("Result: Member data result")),
                It.IsAny<Exception>(),
                It.IsAny<Func<It.IsAnyType, Exception, string>>()),
            Times.Once);
    }
    ...
    private void SetupSuccessfulScenarioWithMemberData()
    {
        ...
        var memberResult = OperationResult<string>.Ok("Member data result");
    ...
    private void SetupMixedResultsScenario()
    {
        ...
        var failureResult1 = OperationResult<string>.Fail("Member operation failed");
        var successResult = OperationResult<string>.Ok(String.Empty);
    ...
    private void VerifyEncryptionCalled()
    {
        _encryptionInfoMock.Verify(x => x.EncryptObjectStrings(It.IsAny<DeleteUserInfoRollback>()), Times.Once);
        _encryptionInfoMock.Verify(x => x.EncryptRsaObjectStrings(It.IsAny<DeleteUserInfoRollback>(), TestPublicKey), Times.Once);
    }
}