...
namespace MessagingSystem.Services.Messaging.WebApi.Messages.Contracts;

public class CreateMessage(string sender, string recipient, string content)
{
    ...
    [RegularExpression("^(?=.*[!_@])[a-zA-Z0-9!_@]{3,15}$", 
        ErrorMessage = "Recipient name must be 3 to 15 characters long and " +
                       "include at least one special character (!, _, @).")]
